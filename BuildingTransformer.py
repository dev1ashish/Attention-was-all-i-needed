...
#         # sin to even and cosine to odd indices
...
#         return self.alpha * (x - mean) / (std + self.eps) + self.bias #formula for layer normalization
...
#     def forward(self, x):
#         #(batch_size, seq_len, d_model) --> (batch_size, seq_len, d_ff) (using leniar 1)--> (batch_size, seq_len, d_model) (using leniar 2)
...
#         attention_scores = (query @ key.transpose(-2, -1)) / math.sqrt(d_k) #(batch_size, num_heads, seq_len, seq_len)
#         if mask is not None:
#             attention_scores = attention_scores.masked_fill(mask == 0, -1e9)
#         attention_scores = torch.nn.functional.softmax(attention_scores, dim = -1) # (batch_size, num_heads, seq_len, seq_len)
...
#         #(batch_size, seq_len, d_model) --> (batch_size, num_heads, seq_len, d_k) --> (batch_size, num_heads, seq_len, d_k)
...
#         return self.out(x) #    (batch_size, seq_len, d_model)
...
#         self.norm = LayerNormalization()
#         # self.linear = nn.Linear(in_features=350, out_features=512) ############################### abhi abhi cheda hai 
...
#         x = self.residual_connection[0](x, lambda x: self.self_attention(x, x, x, tgt_mask)) #i am losing it
#         x = self.residual_connection[1](x, lambda x: self.cross_attention(x, encoder_output, encoder_output, src_mask))
#         x = self.residual_connection[2](x, self._feed_forward)
#         return x
...
# def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int,  d_model: int = 512, N: int = 6 , num_heads: int = 8, dropout: float = 0.1, d_ff = 2048)->Transformer:
#    #hyperparameters as per this stupid paper which took away 2 nights to implement
#    #embedding layers
